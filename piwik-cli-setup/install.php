...
class PiwikCliInstall {
	...
	protected function createUser() {
      ...
      // set the token_auth directly in DB
      if (array_key_exists('token_auth', $config_arr)) {
        $table_name = Common::prefixTable('user');
        $q = "UPDATE {$table_name} SET token_auth='{$config_arr['token_auth']}' WHERE login='{$config_arr['login']}';";
        Db::exec($q);
      }
		});
	}
